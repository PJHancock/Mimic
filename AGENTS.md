...
- **Primary success criterion:** On a held-out human demonstration, the system produces a valid `IDLE -> HOVER -> GRASP -> CARRY -> RELEASE -> HOVER -> IDLE` episode, or the explicitly supported `IDLE -> HOVER -> GRASP -> CARRY -> RELEASE -> IDLE` terminal variant, maps the demonstrated object motion into the Panda workspace, and successfully performs the corresponding pick-and-place in simulation with the object ending near the demonstrated destination.
