...
- **Primary success criterion:** On a held-out human demonstration, the system produces a valid `IDLE -> HOVER -> GRASP -> CARRY -> RELEASE -> HOVER -> IDLE` episode, the explicitly supported `IDLE -> HOVER -> GRASP -> CARRY -> RELEASE -> IDLE` terminal variant, or a subsequent `RELEASE -> IDLE -> GRASP -> CARRY -> RELEASE` continuation, maps the demonstrated object motion into the Panda workspace, and successfully performs every corresponding pick-and-place in one continuous simulation with the object ending near each demonstrated destination.
...
- The persisted post-model robot handoff is `mimic.demo_task_input.v1`. It stores
  catalog/checkpoint/post-processing provenance once, a `resolved_actions`
  sequence with exactly one accepted state per classifier timestep, and an
  independently sampled `object_tracks` sequence. Raw score distributions remain
  optional diagnostics and are never exposed to the robot framework. Derived
  action segments and tracking summaries are not persisted.
...
- `ObjectTrack.table_xy_m` replaces the old image-coordinate `center_2d`: inputs are already calibrated table coordinates in meters, top-left origin, +X right, +Y down. Image calibration remains upstream.
- For each complete single-object episode, `ExtractedTask` uses its GRASP and RELEASE frames as endpoints. Require exact tracking observations at both; reject missing endpoints, invalid ordering, incomplete episodes, and mixed object identities. No state post-processing or interpolation happens in extraction. `extract_tasks` supports multiple adjacent episodes that share a boundary IDLE. After at least one complete episode, a continuation may omit its initial HOVER and begin `IDLE -> GRASP`; the first episode still requires `IDLE -> HOVER -> GRASP`.
...
- `CoordinateRetargeter` requires explicit, robot-independent mapping configuration: named frames, target XY origin in meters, and perpendicular unit source-axis directions in target XY. Source and target coordinates are both meters; never rescale, normalize axes, resize, shear, or clamp positions. The default MuJoCo mapping places the robot/base origin 0.15 m behind the center of the filmed table's left/near long edge, with +X into the table, +Y left, and +Z up.
- The default `tabletop_clone` is a bounded `0.508 m x 0.762 m` MuJoCo box whose near edge is 0.15 m in front of the robot/base marker and whose top surface and base marker are at `z=0`; its `0.01 m` thickness is a minimal simulation default below the surface, not a measured physical property or a Panda assumption.
...
- **Default episode:** `IDLE -> HOVER -> GRASP -> CARRY -> RELEASE -> HOVER -> IDLE`. A successfully observed episode may omit the terminal HOVER and end `RELEASE -> IDLE`. After a complete episode, `RELEASE -> IDLE -> GRASP` is an explicitly supported continuation; an initial `IDLE -> GRASP` remains invalid. IDLE is a legal terminal transition from every state, emits no learned-label arm or gripper command, and an earlier transition to IDLE terminates an incomplete/aborted episode rather than making it executable. `HOVER` means move to grasp hover when entered from IDLE, and return to the configured home joint preset when entered from RELEASE. `GRASP -> HOVER` is an abort-to-home edge guarded by an empty grasp. A missed continuation HOVER is an extraction exception only; robot playback still generates an approach/hover from the next grasp waypoint before descending. Multi-episode playback does not return home between successful episodes. The next episode's HOVER realizes its approach pose on a home-seeded IK branch through the bounded joint-reference path, then confirms Cartesian arrival. Object initialization happens only before episode one; MuJoCo robot and object state remain continuous thereafter.
...
- Reset-only simulation initialization places the named free-joint object at the selected task's retargeted grasp pose before physics starts. This is initialization, not execution-time teleportation; the latter remains prohibited.
