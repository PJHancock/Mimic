...
const isStaticShowcase = import.meta.env.PROD
const baseUrl = import.meta.env.BASE_URL

function dataUrl(path: string) {
  return isStaticShowcase ? `${baseUrl}demo/${path.replace(/^\//, '')}.json` : `/api/${path.replace(/^\//, '')}`
}

function publicAssetUrl(path: string | null) {
  if (!path || !isStaticShowcase || /^(?:https?:)?\/\//.test(path)) return path
...
const flowboxData: Record<string, FlowboxInfo> = {
  video: { title: '📹 VIDEO INPUT', technical: 'Raw video frames from human demonstrations', skills: ['RGB-D data', 'Dataset inspection', 'Sensory inputs'], video: `${baseUrl}input-demo.webm` },
  ...
  embeddings: { title: '🧠 EMBEDDINGS', technical: 'ResNet50 self-supervised visual feature extraction from frames (V-JEPA functionality included)', skills: ['Representation learning', 'VLA models', 'PyTorch/JAX'] },
  classifier: { title: '⏱️ CLASSIFIER', technical: 'LSTM model predicts manipulation phases from temporal embeddings', skills: ['Imitation learning', 'Policy learning', 'PyTorch'], image: `${baseUrl}loss_curves.png` },
  ...
  postproc: { title: '📋 POST PROCESSING & SKILL GRAPH', technical: 'Filter noise, smooth predictions, and build skill state transition graph', skills: ['Signal processing', 'Graph algorithms', 'Sequence planning', 'State transitions'], video: `${baseUrl}sidebyside.webm` },
...
export default function App() {
  ...
  useEffect(() => { fetch(dataUrl('runs')).then((r) => { if (!r.ok) throw new Error('Could not read run artifacts.'); return r.json() }).then((data: Run[]) => { setRuns(data); if (data[0]) setRunId(data[0].id); else { setLoading(false); setError('No result artifacts were found.') } }).catch((cause) => { setLoading(false); setError(String(cause)) }) }, [])
  useEffect(() => { if (isStaticShowcase) return; Promise.all([fetch('/api/raw-videos').then((r) => r.json()), fetch('/api/robot-configs').then((r) => r.json()), fetch('/api/process').then((r) => r.json())]).then(([videos, configs, job]: [RawVideo[], RobotConfig[], PipelineJob]) => { setRawVideos(videos); setSelectedRaw((current) => current || videos[0]?.name || ''); setRobotConfigs(configs); setSelectedConfig((current) => current || configs.find((item) => item.default)?.id || configs[0]?.id || ''); setPipelineJob(job) }).catch((cause) => setPipelineError(String(cause))) }, [])
  useEffect(() => { if (!runId) return; setLoading(true); setProgress(0); setPhaseSync(false); setSyncPlaying(false); fetch(dataUrl(`runs/${encodeURIComponent(runId)}`)).then((r) => { if (!r.ok) throw new Error('Run artifacts could not be loaded.'); return r.json() }).then((data: Detail) => { setDetail({ ...data, source_video_url: publicAssetUrl(data.source_video_url), simulation_video_url: publicAssetUrl(data.simulation_video_url) }); setLoading(false); setError('') }).catch((cause) => { setLoading(false); setError(String(cause)) }) }, [runId, artifactRevision])
  useEffect(() => {
  ...
  return <div className="app-shell">
    <header className="topbar"><div className="brand"><div className="mark">M</div><div><strong>MIMIC</strong><span>INTERACTIVE PIPELINE</span></div></div><div className="pipeline"><button onClick={() => document.getElementById('section-overview')?.scrollIntoView({ behavior: 'smooth' })} style={{background: 'none', border: 'none', color: 'inherit', cursor: 'pointer', font: 'inherit', padding: 0}}>PROJECT OVERVIEW</button><i>→</i><button onClick={() => document.getElementById('section-pipeline')?.scrollIntoView({ behavior: 'smooth' })} style={{background: 'none', border: 'none', color: 'inherit', cursor: 'pointer', font: 'inherit', padding: 0}}>TECHNOLOGY PIPELINE</button><i>→</i><button onClick={() => document.getElementById('section-demo')?.scrollIntoView({ behavior: 'smooth' })} style={{background: 'none', border: 'none', color: 'inherit', cursor: 'pointer', font: 'inherit', padding: 0}}>DEMO</button></div><div style={{display: 'flex', alignItems: 'center', gap: '16px', marginLeft: 'auto'}}><div style={{textAlign: 'right', fontSize: '9px', letterSpacing: '.08em', lineHeight: '1.5', fontFamily: '"SFMono-Regular", Consolas, monospace'}}><div style={{color: 'var(--muted)', marginBottom: '4px'}}>Preston Hancock · Josh McConkie · Peter Bickel</div><a href="https://github.com/PJHancock/Mimic" target="_blank" rel="noopener noreferrer" style={{color: 'var(--cyan)', textDecoration: 'none', cursor: 'pointer', fontSize: '9px'}}>GitHub Repository</a></div><button onClick={toggleTheme} style={{background: 'none', border: 'none', color: 'inherit', cursor: 'pointer', font: 'inherit', padding: '0 8px', fontSize: '16px', display: 'flex', alignItems: 'center'}} title="Toggle theme">{theme === 'dark' ? '☀️' : '🌙'}</button></div></header>
